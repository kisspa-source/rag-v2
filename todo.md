...
- [ ] Ollama 서비스 확인: `http://localhost:11434` 접속
...
- [ ] CLI로 질문 수행 테스트:
  ```bash
  python -m rag_engine.cli "테스트 질문"
  ```
- [ ] 샘플 PDF 로드 → 인덱싱 → 질문 → 답변 생성 테스트:
  ```python
  engine = RAGEngine(config)
  engine.load_documents(["sample.pdf"])
  engine.index_documents()
  answer = engine.run("문서의 주요 내용은?")
  print(answer)
  ```
- [ ] 답변에 출처 포함 확인
...
- [ ] **LLM 설정**:
  - Temperature (0.0~1.0)
  - Max Tokens (100~1000)
- [ ] **System Prompt 편집기** (`st.text_area`)
- [ ] **설정 저장/로드** (`config.yaml`)
- [ ] **파일 관리**:
  - 인덱싱된 파일 목록 표시
...
### ✅ TEST 3.4: 관리자 기능 검증
- [ ] 파라미터 변경 후 저장 확인
- [ ] `config.yaml` 파일 업데이트 확인
- [ ] System Prompt 수정 및 적용 확인
- [ ] Temperature 변경 효과 확인
- [ ] 파일 목록 표시 확인
- [ ] 파일 삭제 동작 확인
...
### 5.1 메모리 최적화
- [ ] **모델 캐싱** 확인 (`@st.cache_resource`)
- [ ] **메모리 사용량 모니터링** 코드 추가 (선택사항)
- [ ] **Garbage Collection** 명시적 호출 (대용량 파일 처리 후)

### 5.2 성능 프리셋 구현
- [ ] **환경별 설정 추가** (`config.yaml`):
  - **Preset 8GB (저사양)**:
    - Rerank: OFF
    - Context Count: 2
    - Chunk Size: 600
...
### 6.1 평가 데이터 세트 생성
- [ ] `eval/qa_pairs.jsonl` 파일 생성
- [ ] 테스트 질문-답변 쌍 작성 (최소 10개):
  ```json
...
**현재 진행해야 할 순서**:
1. Phase 0 (환경 설정) → TEST 0
2. Phase 1 (MVP 패키지) → TEST 1
3. Phase 2 (RAG 엔진) → TEST 2.1~2.6
4. Phase 3 (Streamlit UI) → TEST 3.1~3.5
5. Phase 4 (보안) → TEST 4
6. Phase 5 (성능) → TEST 5
7. Phase 6 (평가) → TEST 6
8. Phase 7 (Advanced, 선택) → TEST 7
9. Phase 8 (통합 테스트) → TEST 8.1~8.3
10. Phase 9 (배포) → TEST 9
